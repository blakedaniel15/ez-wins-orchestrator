...
## Crons disabled until cutover
Orchestrator sweep/cadence crons removed from vercel.json (2026-07-01) so the orchestrator does NOT auto-sweep while the email assistant is still live (shared EZ-Assistant-Processed tag causes interference). Endpoints remain manually triggerable (CRON_SECRET or logged-in admin). Re-add crons at cutover.
